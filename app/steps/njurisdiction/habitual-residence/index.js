...
module.exports = class JurisdictionHabitualResidence extends OptionStep {
  get url() {
    return '/njurisdiction/habitual-residence';
  }
  ...
  get nextStep() {
    return {
      ...
      jurisdictionWhereTo: {
        ...
        JurisdictionNDomicile: this.steps.JurisdictionNDomicile
      }
    };
  }
  ...
  next(ctx, session) {
    ...
    if (areBothHabituallyResident(this.name, ctx, session) || isHabitualResident('respondent', this.name, ctx, session)) {
      ...
      nextStepLogicHelper.jurisdictionWhereTo = 'JurisdictionNDomicile';
    }
